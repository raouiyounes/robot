...
int main (int argc, char** argv) {
    ...
    std::string topic_root = "stlucia";

    // set most important visual odometry parameters
    VisualOdometryMono::parameters param;

    param.calib.f  = 641.8;//641.8;721.5377;//718.856;//707.1; // focal length in pixels
    param.calib.cu = 220.7;//228.7;//609.5593;//607.19;//601.9; // center point in pixels
    param.calib.cv = 242.4;//236.4;//236.4;172.854;//185.2157;//183.11; // center point in pixels
    param.height = 1.0;     // in meters
    
    viso = new VisualOdometryMono(param);

    fichier1.open("~/Documents/testoo/stlucia_vo1_traj.txt", ios::out | ios::trunc);
    fichier2.open("~/Documents/testoo/stlucia_vo1_poses.txt", ios::out | ios::trunc);
